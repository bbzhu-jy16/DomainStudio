...
def main(args):
    ...
    unet_source = UNet2DConditionModel.from_pretrained(
        args.pretrained_model_name_or_path,
        subfolder="unet",
        revision=args.revision,
        torch_dtype=torch.float32
    ...
    unet_source = unet_source.to(accelerator.device)
    ...
    prior_steps = 500
    prior_epochs = 1
    progress_bar_prior = tqdm(range(prior_steps), disable=not accelerator.is_local_main_process)
    progress_bar_prior.set_description("Steps")
            ...
            with accelerator.accumulate(unet):
                ...
                with torch.no_grad():
                    if not args.not_cache_latents:
                        ...
                        latent_dist = vae.encode(batch["pixel_values"].to(accelerator.device, dtype=weight_dtype)).latent_dist
                ...
                if args.with_prior_preservation:
                    ...
                    loss = loss + args.prior_loss_weight * prior_loss + args.image_loss_weight * rel_loss \
